...
return {
    ...
    parse_feature: function( line ) {
        ...
        parsed.ref_contig_id = f[2].trim();
        parsed.query_contig_id = f[1].trim();
        parsed.type = "QueryContig";
        parsed.qstart = parseInt(f[3].trim(),10);
        parsed.qend = parseInt(f[4].trim(),10);
        parsed.start = parseInt(f[5].trim(),10);
        parsed.end = parseInt(f[6].trim(),10);
        parsed.score = null;
        parsed.phase = 0;
        parsed.strand = f[7].trim();
        //parsed.strand = null;
        parsed.alignment = f[13].trim();
        parsed.name = f[1].trim();
        parsed.attributes = { "ID":[f[1].trim()]};
        return parsed;
    },
